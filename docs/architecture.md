...
The snapshot has an explicit schema version and contains no tokens, pairing codes, credentials, headers, or raw discovery JSON. Pairing metadata is persisted separately from this snapshot and the encrypted token. Startup asynchronously resolves the snapshot plus secure session, verifies `/auth/me`, and only then routes to Products. Change Store clears the local session records before clearing this discovered snapshot.
